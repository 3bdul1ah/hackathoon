"""Emirates ID plus email OTP identity verification (mocked, multi-factor).

This runs BEFORE the dispute is processed: a customer must prove who they are
with their Emirates ID, registered email, and a one-time passcode before the
agent will look at any order or payment. The agent never sees real KYC data
here; all checks are mocked against the customer record, but the *flow* mirrors
a real step-up KYC: format check, identity match, email match, OTP.
...
def verify(customer: dict[str, Any] | None, *, provided_emirates_id: str | None,
           provided_email: str | None, otp_verified: bool) -> dict[str, Any]:
    """Return a structured verification result.

    factors: each named check with pass/fail. `verified` is True only when the
    Emirates ID format is valid, it matches the account on file, the email
    matches the account on file, and the OTP check passed.
    """
    customer = customer or {}
    on_file = customer.get("emirates_id")
    email_on_file = customer.get("email")
    ...
    email_ok = bool(provided_email and email_on_file
                    and provided_email.strip().casefold() == str(email_on_file).strip().casefold())
    ...
    factors = [
        ...
        {"factor": "Registered email matches account on file", "passed": email_ok},
        {"factor": "One-time passcode (email OTP) to registered email", "passed": bool(otp_verified)},
    ]

    verified = fmt_ok and match_ok and email_ok and bool(otp_verified)

    if not verified:
        detail = "Identity NOT verified: " + ", ".join(
            f["factor"] for f in factors if not f["passed"]) + " failed."
    elif email_recently_changed:
        detail = ("Identity verified via Emirates ID + registered email + email OTP. "
                  "Note: a recent email change is flagged for the reviewer.")
    else:
        detail = "Identity verified via Emirates ID + registered email + email OTP."
